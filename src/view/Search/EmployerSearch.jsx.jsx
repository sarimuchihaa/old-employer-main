...
const EmployerSearch = () => {
  ...
  useEffect(() => {
    if (inputs.country) {
      const filtered = cities.filter(
        (city) => city.country_name === inputs.country
      );
  ...
  useEffect(() => {
    ...
    const countryNames = searchParams.get("country")
  ...
  const handleRemoveIndustry = (industryToRemove) => {
    setInputs((prevInputs) => ({
      ...prevInputs,
      industry: prevInputs.industry.filter(
        ...
        <div className={styles["columns-container"]}>
          <div className={styles["left-column"]}>
            <div className={styles["filter-group"]}>
              <div
                className={`${styles["saved-searches-dropdown"]} ${
                  isOpen ? styles["open"] : ""
                }`}
              >
              ...
              <label className={styles["filters-search"]}>
                {inputs?.country && (
                  <span className={styles["filter-pill"]}>
                    ...
                    <span
                      ...
                      onClick={() => handleRemoveCountry(inputs.country)}
                    >
                      ✕
                    </span>
                  </span>
                ...
                {inputs?.industry?.length > 0 &&
                  inputs.industry.map((industry, index) => (
                  ...
                  {sizes.map((size) => (
                    <div key={size}>
                      <input
                        type="checkbox"
                        value={size}
                        checked={inputs.size.includes(size)}
                  ...
                  {ratings.map((rating) => (
                    <div key={rating}>
                      <input
                        ...
                        name="overallRating"
                        value={rating}
                        checked={inputs.overallRating === rating}
                        onChange={() => handleOverallRatingChange(rating)}
                    ...
                    const stars = Array.from({ length: 5 }, (_, index) => (
                      <span
                        ...
                        style={{
                          cursor: "pointer",
                          color:
                            index < company.calculatedOverallRating
                              ? "#FC9823"
                              : "#ccc",
                        }}
                      >
